...
#define VNDUSERIALDBG(param, ...) {HILOGD(param, ## __VA_ARGS__);}
#else
...
/*    if (p_cfg->fmt & USERIAL_DATABITS_8) {
    ...
    if (!((p_cfg->fmt & USERIAL_DATABITS_8)||(p_cfg->fmt & USERIAL_DATABITS_7)||
	(p_cfg->fmt & USERIAL_DATABITS_6)||(p_cfg->fmt & USERIAL_DATABITS_5))) {
...
/*    if (p_cfg->fmt & USERIAL_PARITY_NONE) {
    ...
    if (!((p_cfg->fmt & USERIAL_PARITY_NONE)||(p_cfg->fmt & USERIAL_PARITY_EVEN)||(p_cfg->fmt & USERIAL_PARITY_ODD))) {
    ...
    HILOGI("userial vendor open: opening %s, baud %d", vnd_userial.port_name,
          p_cfg->baud);
    ...
    if (vnd_userial.fd == -1)
    {
        while(vnd_userial.fd == -1 && count_id --)
        {
            ...
            HILOGI("vendor open bt device fail,try time: %d", count_id);
            vnd_userial.fd = open(vnd_userial.port_name, O_RDWR);
        }
    }
    if (vnd_userial.fd == -1) {
        HILOGE("userial vendor open: unable to open %s errno %d %s", vnd_userial.port_name, errno, strerror(errno));
        return -1;
    }
    ...
    char value[92] = "userdebug";
    if (strstr(value, "userdebug")) {
        HILOGI("%s set usrdebug mode to tty.", __func__);
        int fd = -1, ret;
        char value_usr_debug = 2;
        //write to sipc node
        fd = open(SPIC_SYSFS_DUMPMEM_NODE, O_WRONLY);
        if (fd < 0) {
          HILOGI("open %s faild, error=%s (%d)", SPIC_SYSFS_DUMPMEM_NODE, strerror(errno), errno);
        } else {
          ret = write(fd, &value_usr_debug, 1);
          if (ret < 0) {
            HILOGI("write %s faild, error=%s (%d)", SPIC_SYSFS_DUMPMEM_NODE, strerror(errno), errno);
          }
          close(fd);
        }
        //write to sdio node
        ...
        if (fd < 0) {
          HILOGI("open %s faild, error=%s (%d)", SDIO_SYSFS_DUMPMEM_NODE, strerror(errno), errno);
        } else {
          ret = write(fd, &value_usr_debug, 1);
          if (ret < 0) {
            HILOGI("write %s faild, error=%s (%d)", SDIO_SYSFS_DUMPMEM_NODE, strerror(errno), errno);
          }
          close(fd);
        }
    }
...
void userial_vendor_close(void)
{
    ...
    if (result < 0) {
        HILOGE("tcgetattr(fd:%d) FAILED result:%d", vnd_userial.fd, result);
    } else {
        term.c_cflag &= ~(CRTSCTS);
        HILOGI("device fd = %d CRTSCTS clear", vnd_userial.fd);
        result = tcsetattr(vnd_userial.fd, TCSANOW, &term);
        if (result < 0) {
            HILOGE("tcsetattr(fd:%d) FAILED result:%d", vnd_userial.fd, result);
        }
    }
    ...
    HILOGI("device fd = %d clear", vnd_userial.fd);
