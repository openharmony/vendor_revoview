...
#if (UPIO_DBG == TRUE)
#define UPIODBG(param, ...) {HILOGD(param, ## __VA_ARGS__);}
#else
...
static int connect_wcnd(void)
{
    ...
    while (client_fd < 0 && retry_count > 0) {
        retry_count--;
        HILOGD("bt-vendor : %s: Unable bind server %s, waiting...\n", __func__,
              WCND_SOCKET_NAME);
        usleep(100 * 1000);
        client_fd = socket_local_client(
                        WCND_SOCKET_NAME, ANDROID_SOCKET_NAMESPACE_ABSTRACT, SOCK_STREAM);
    ...
    if (setsockopt(client_fd, SOL_SOCKET, SO_RCVTIMEO, (char *)&rcv_timeout,
        ...
        HILOGE("bt-vendor : %s: set receive timeout fail\n", __func__);
    }

    HILOGD("bt-vendor : %s: connect to server status:%d\n", __func__, client_fd);
...
static int start_cp2(void)
{
    ...
    if (wcnd_socket < 0) {
        HILOGD("bt-vendor : %s: connect to server failed", __func__);
        return -1;
    }

    len = strlen(WCND_BT_CMD_STR_START_CP2) + 1;

    HILOGD("bt-vendor : %s: send start cp2 message to wcnd %s\n", __func__,
          WCND_BT_CMD_STR_START_CP2);

    ret = TEMP_FAILURE_RETRY(write(wcnd_socket, WCND_BT_CMD_STR_START_CP2, len));

    if (ret <= 0) {
        HILOGD("bt-vendor : %s: write to wcnd fail.", __func__);
        close(wcnd_socket);
        return ret;
    }
    ...
    HILOGD("bt-vendor : %s: waiting for server %s\n", __func__, WCND_SOCKET_NAME);
    ret = read(wcnd_socket, buffer, 128);

    HILOGD("bt-vendor : %s: get %d bytes %s\n", __func__, ret, buffer);
...
static int stop_cp2(void)
{
    ...
    HILOGD("bt-vendor : %s: send stop cp2 message to wcnd %s\n", __func__,
...
void upio_set(uint8_t pio, uint8_t action, uint8_t polarity)
{
    ...
    case UPIO_BT_WAKE:
...
#endif
        if (fd_btwrite < 0) {
            //HILOGE("upio_set : %s(%d) incorrent", VENDOR_BTWRITE_PROC_NODE,
            //      fd_btwrite);
            return;
        }
        ...
        if (write(fd_btwrite, &buffer, 1) < 0) {
            //HILOGE("upio_set : write(%s) failed: %s (%d)", VENDOR_BTWRITE_PROC_NODE,
