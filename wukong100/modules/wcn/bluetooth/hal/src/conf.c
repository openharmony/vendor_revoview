...
static int read_mac_address(char *file_name, uint8_t *addr)
{
    ...
    HILOGI("%s read file: %s", __func__, file_name);
    if (fd < 0) {
        HILOGI("%s open %s error reason: %s", __func__, file_name, strerror(errno));
        return -1;
    }
    if (read(fd, buf, MAC_ADDR_BUF_LEN) < (int)MAC_ADDR_BUF_LEN) {
        HILOGI("%s read %s error reason: %s", __func__, file_name, strerror(errno));
        goto error;
    }
    if (sscanf(buf, "%02X:%02X:%02X:%02X:%02X:%02X", &addr_t[0], &addr_t[1],
               &addr_t[2], &addr_t[3], &addr_t[4], &addr_t[5]) < 0) {
        HILOGI("%s sscanf %s error reason: %s", __func__, file_name,
              strerror(errno));
        goto error;
    }
    ...
    HILOGI("%s %s addr: [%02X:%02X:%02X:%02X:%02X:%02X]", __func__, file_name,
...
void set_mac_address(uint8_t *addr)
{
    ...
    uint8_t addr_t[6] = {0};
    int fd;
    char buf[MAC_ADDR_BUF_LEN+1];

    HILOGI("%s", __func__);
    /* check misc mac file exist */
    ret = access(DATMISC_MAC_ADDR_PATH, F_OK);
    if (ret != 0) {
        HILOGI("%s %s miss", __func__, DATMISC_MAC_ADDR_PATH);
        random_mac_addr_create(addr_t);
        HILOGI("%s, Random MAC address: %02X:%02X:%02X:%02X:%02X:%02X\n", __func__,
            addr_t[0], addr_t[1], addr_t[2], addr_t[3], addr_t[4], addr_t[5]);
        fd = open(DATMISC_MAC_ADDR_PATH, O_CREAT | O_RDWR | O_TRUNC, 0666);
        if (fd >= 0) {
            HILOGI("%s create success\n", DATMISC_MAC_ADDR_PATH);
            sprintf(buf, "%02x:%02x:%02x:%02x:%02x:%02x", addr_t[0], addr_t[1], addr_t[2],
                    addr_t[3], addr_t[4], addr_t[5]);
            write(fd, buf, MAC_ADDR_BUF_LEN);
            close(fd);
        } else {
            HILOGI("%s create fail:fd=%d, reason is %s\n", DATMISC_MAC_ADDR_PATH, fd, strerror(errno));
        }
         /* compose mac stream */
    ...
    if (ret != 0) {
        HILOGI("%s %s error", __func__, DATMISC_MAC_ADDR_PATH);
        return;
    }
