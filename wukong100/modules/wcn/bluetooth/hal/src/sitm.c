...
#if (SITM_DBG == TRUE)
#define SITMD(param, ...) HILOGD("%s "param, __FUNCTION__, ## __VA_ARGS__)
#else
#define SITMD(param, ...) {}
#endif
#define SITME(param, ...) HILOGE("%s "param, __FUNCTION__, ## __VA_ARGS__)
...
static int open_sco_userial(char *name) {
    ...
    if (fd < 0) {
        HILOGE("open %s failed: %s", name, strerror(errno));
        return fd;
    }
...
static int serial_data_ready(uint8_t *data, uint32_t len) {
    ...
    if (len > 7)  {
        HILOGD("R[%u]: %02X%02X%02X%02x %02X%02X%02X%02X", len,
