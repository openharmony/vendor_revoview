...
void sipc_free_tx_buf(struct sprd_hif *hif, struct sprd_msg *msg_buf)
{
	...
	struct sipc_buf_node *node = (struct sipc_buf_node *)msg_buf->sipc_node;
	...
	if (unlikely(msg_buf->mode >= SPRD_MODE_MAX)) {
		wl_err("%s: invalid mode:%u\n", __func__, msg_buf->mode);
		return;
	}
	tx_mgmt = (struct tx_mgmt *)hif->tx_mgmt;
	atomic_inc(&tx_mgmt->sipc_tx_cred[msg_buf->mode]);
}
				...
				struct sprd_msg *msg_pos)
{
	...
	if (unlikely(msg_pos->mode >= SPRD_MODE_MAX)) {
		wl_err("%s,%d invalid mode:%u\n", __func__, __LINE__, msg_pos->mode);
		return -1;
	}
	...
	if (node->buf < tx_buf->virt_start || node->buf > tx_buf->virt_end) {
		wl_err("%s node buf is err\n", __func__);
		return -1;
	}

	atomic_dec(&tx_mgmt->sipc_tx_cred[msg_pos->mode]);
