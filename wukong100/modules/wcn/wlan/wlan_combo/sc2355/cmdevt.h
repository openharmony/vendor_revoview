...
#endif
#define SYNC_LUT_VERSION	7

#define NOTIFY_AP_VERSION_USER 0
#define NOTIFY_AP_VERSION_USER_DEBUG 1

#define SYNC_LUT_VERSION_SAP_STA_NOT_COEX	0
#define SYNC_LUT_VERSION_SAP_STA_COEX	1
...
struct cmd_set_scan_power_backoff {
#define SPRD_SET_SCAN_POWER_BACKOFF 0x12
	u8 power_save_type;
	u8 num;
	struct sprd_power_backoff backoff[];
} __packed;
...
struct cmd_set_band_sar_value {
#define SPRD_SET_BAND_SAR 0x13
        u8 power_save_type;
        u8 type;
        u8 sar_value[5];
} __packed;
...
/* set reduce power subtype
 * SET_SAR_POWER: set sar command.
 * SET_POWER_BACKOFF: set power backoff commond.
 * SET_5G_POWER_BACKOFF: set 5g power backoff base band1, band2, band3, band4.
 */
enum reduce_power_subtype {
	SET_SAR_POWER = 1,
	GET_SAR_POWER = 2,
	SET_POWER_BACKOFF = 3,
	GET_POWER_BACKOFF = 4,
	SET_5G_POWER_BACKOFF = 5,
	SET_APC_STATUS = 6,
        SET_APC_MODE = 7,
	GET_APC_MODE = 8,
        SET_CLIENT_POWER = 9,
        GET_CLIENT_POWER = 10,
	REDUCE_POWER_SUBTYPE_MAX,
};

#define REDUCE_POWER_PARA_MAX_LEN	32
struct reduce_power {
	u8 type;
	u8 len;
	u8 value[REDUCE_POWER_PARA_MAX_LEN];
} __packed;
...
struct chan_changed_info_previous {
	u8 initiator;
	u8 target_channel;      /* primary channel */
} __packed;

struct chan_changed_info {
	u8 initiator;
	u8 target_channel;      /* primary channel */
	u8 ch_width;            /* see enum chan_changed_bw */
	u8 center_channel;      /* center_channel */
} __packed;
...
{
	/*2953043,2858800*/
	if (priv->hif.suspend_mode == SPRD_PS_SUSPENDED && priv->probe_done
	    && !rbuf && priv->hif.hw_type == SPRD_HW_SC2355_SDIO)
		return sc2355_send_cmd_recv_rsp_async(priv, msg, rbuf, rlen);
