...
#else
	wl_info(" %s, (%02x:%02x:%02x:%02x:xx:xx)(%s)%u, chn %2u, sig %d, freq %u, ie_chn(%d)\n",
		ieee80211_is_probe_resp(mgmt->frame_control) ? "proberesp" : "beacon",
		mgmt->bssid[0], mgmt->bssid[1], mgmt->bssid[2], mgmt->bssid[3],
		ssid, ssid_len, chan, signal, freq, ie_channel_number);
#endif
