...
/* get iface concurrency matrix function---CMD ID:0x10001 */
static const struct wifi_iface_limit vnd_iface_limits[] = {
	{
		.max_limit = 1,
...
static const struct wifi_iface_limit vnd_iface_limits1[] = {
	{
		.max_limit = 1,
...
static const struct wifi_iface_combination vnd_iface_combos[] = {
	{
		.max_ifaces = 2,
		.num_iface_limits = ARRAY_SIZE(vnd_iface_limits),
		.iface_limits = vnd_iface_limits
	},
	{
		.max_ifaces = 1,
		.num_iface_limits = ARRAY_SIZE(vnd_iface_limits1),
		.iface_limits = vnd_iface_limits1
	}
};

static const struct wifi_iface_concurrency_matrix vnd_iface_matrix = {
	.num_iface_combinations = ARRAY_SIZE(vnd_iface_combos),
	.iface_combinations = vnd_iface_combos
};

/*sta+ap coex*/
static const struct wifi_iface_limit vnd_iface_limits_sta_ap[] = {
	{
		.max_limit = 1,
		.iface_mask = BIT(WIFI_INTERFACE_TYPE_STA)
	},
	{
		.max_limit = 1,
		.iface_mask = BIT(WIFI_INTERFACE_TYPE_P2P) |
				BIT(WIFI_INTERFACE_TYPE_AP)
	}
};

static const struct wifi_iface_combination vnd_iface_combos_sta_ap[] = {
	{
		.max_ifaces = 2,
		.num_iface_limits = ARRAY_SIZE(vnd_iface_limits_sta_ap),
		.iface_limits = vnd_iface_limits_sta_ap
	}

};

static const struct wifi_iface_concurrency_matrix vnd_iface_matrix_sta_ap = {
	.num_iface_combinations = ARRAY_SIZE(vnd_iface_combos_sta_ap),
	.iface_combinations = vnd_iface_combos_sta_ap
};

static int vendor_fill_concurrency_matrix(struct sk_buff *reply,
					  const struct wifi_iface_concurrency_matrix *matrix)
{
	struct nlattr *nl_matrix, *nl_combos;
	int i, j;

	nl_matrix = nla_nest_start(reply, ATTR_IF_CO_MATRIX);
	if (!nl_matrix)
		goto out_put_fail;

	if (nla_put_u32(reply, ATTR_IF_CO_NUM_IFACE_COMBOS, matrix->num_iface_combinations))
		goto out_put_fail;

	nl_combos = nla_nest_start(reply, ATTR_IF_CO_IFACE_COMBOS);
	if (!nl_combos)
		goto out_put_fail;

	for (i = 0; i < matrix->num_iface_combinations; i++) {
		struct nlattr *nl_combo, *nl_limits;
		const struct wifi_iface_combination *temp_combo;

		temp_combo = &matrix->iface_combinations[i];

		nl_combo = nla_nest_start(reply, i + 1);
		if (!nl_combo)
			goto out_put_fail;

		if (nla_put_u32(reply, ATTR_IF_COMBO_MAX_IFACES, temp_combo->max_ifaces))
			goto out_put_fail;
		if (nla_put_u32(reply, ATTR_IF_COMBO_NUM_IFACE_LIMITS,
				temp_combo->num_iface_limits))
			goto out_put_fail;

		nl_limits = nla_nest_start(reply, ATTR_IF_COMBO_LIMITS);
		if (!nl_limits)
			goto out_put_fail;

		for (j = 0; j < temp_combo->num_iface_limits; j++) {
			struct nlattr *nl_limit;
			const struct wifi_iface_limit *temp_limit;

			temp_limit = &temp_combo->iface_limits[j];

			nl_limit = nla_nest_start(reply, j + 1);
			if (!nl_limit)
				goto out_put_fail;
			if (nla_put_u32(reply, ATTR_IF_LIMIT_MAX_LIMIT, temp_limit->max_limit))
				goto out_put_fail;
			if (nla_put_u32(reply, ATTR_IF_LIMIT_MASK, temp_limit->iface_mask))
				goto out_put_fail;
			nla_nest_end(reply, nl_limit);
		}
		nla_nest_end(reply, nl_limits);
		nla_nest_end(reply, nl_combo);
	}
	nla_nest_end(reply, nl_combos);
	nla_nest_end(reply, nl_matrix);
...
static int vendor_get_iface_concurrency_matrix(struct wiphy *wiphy,
					       struct wireless_dev *wdev,
					       const void *data, int len)
{
	int ret, buf_len;
	...
	const struct wifi_iface_concurrency_matrix *iface_concurrency_matrix;
	u32 n_limits, n_combos;

	if (sta_ap_coex)
		iface_concurrency_matrix = &vnd_iface_matrix_sta_ap;
	else
		iface_concurrency_matrix = &vnd_iface_matrix;

	n_combos = iface_concurrency_matrix->num_iface_combinations;
	n_limits = iface_concurrency_matrix->iface_combinations->num_iface_limits;
	buf_len = sizeof(struct wifi_iface_concurrency_matrix) +
			n_combos * sizeof(struct wifi_iface_combination) +
			n_limits * sizeof(struct wifi_iface_limit) + 0x100;

	reply = cfg80211_vendor_cmd_alloc_reply_skb(wiphy, buf_len);
	if (!reply)
		return -ENOMEM;

	ret = vendor_fill_concurrency_matrix(reply, iface_concurrency_matrix);
	if (ret) {
		kfree_skb(reply);
		reply = NULL;
		return ret;
...
/* get iface name function---CMD ID:0x10002 */
static const char *iftype_to_ifname(struct sprd_priv *priv,
				    u32 iftype)
{
	u8 sta_ap_coex = GET_STA_AP_COEX(priv);
...
static int vendor_reply_iface_name(struct wiphy *wiphy,
				   enum nl80211_iftype iftype)
{
	struct sk_buff *reply = NULL;
	int buf_len = 0, ret;
	struct sprd_priv *priv = wiphy_priv(wiphy);
	const char *ifname = iftype_to_ifname(priv, iftype);

	buf_len = NLA_HDRLEN + IFNAMSIZ;

	reply = cfg80211_vendor_cmd_alloc_reply_skb(wiphy, buf_len);
	if (!reply)
		return -ENOMEM;

	if (nla_put_string(reply, ATTR_IFACE_NAME, ifname))
		goto out_put_fail;

	wl_info("%s,iftype=%u,ifname=%s\n", __func__, iftype, ifname);
...
static int vendor_get_iface_name(struct wiphy *wiphy,
				 struct wireless_dev *wdev,
				 const void *data, int len)
{
	struct nlattr *tb[ATTR_IFACE_NAME_MAX + 1];
	u32 iftype = 0;
	int ret = 0;

	if (nla_parse(tb, ATTR_IFACE_NAME_MAX, data, len, NULL, NULL)) {
		wl_err("%s parse attr failed", __func__);
		return -EINVAL;
	}

	iftype = nla_get_u32(tb[ATTR_IFACE_TYPE]);

	ret = vendor_reply_iface_name(wiphy, iftype);
...
static const struct wiphy_vendor_command vendor_cmd[] = {
	...
	{/* 0x10001 */
		{
			.vendor_id = OUI_SPREAD,
			.subcmd = VENDOR_CMD_GET_IFACE_CONCURRENCY_MATRIX,
		},
		.flags = WIPHY_VENDOR_CMD_NEED_WDEV,
		.doit = vendor_get_iface_concurrency_matrix,
		.policy = VENDOR_CMD_RAW_DATA,
	},
	{/* 0x10002 */
		{
			.vendor_id = OUI_SPREAD,
			.subcmd = VENDOR_CMD_GET_IFACE_NAME,
		},
		.flags = WIPHY_VENDOR_CMD_NEED_WDEV,
		.doit = vendor_get_iface_name,
		.policy = vendor_iface_name_policy,
		.maxattr = ATTR_IFACE_NAME_MAX,
	},
...
int sc2355_vendor_init(struct wiphy *wiphy)
{
	...
	priv->phy_param = kzalloc(sizeof(struct phy_ini_param), GFP_KERNEL);
	if (!priv->phy_param)
		return -ENOMEM;

	priv->mac_param = kzalloc(sizeof(struct mac_ini_param), GFP_KERNEL);
	if (!priv->mac_param){
		kfree(priv->phy_param);
		priv->phy_param = NULL;
		return -ENOMEM;
	}
