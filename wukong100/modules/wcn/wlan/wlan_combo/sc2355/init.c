...
void sc2355_setup_wiphy(struct wiphy *wiphy, struct sprd_priv *priv)
{
	...
	if (priv->hif.hw_type == SPRD_HW_SC2355_SDIO &&
	    priv->extend_feature & SPRD_EXTEND_FEATURE_WPA3_11R) {
		wiphy->iftype_akm_suites = sprd_iftype_akm_suites;
		wiphy->num_iftype_akm_suites = ARRAY_SIZE(sprd_iftype_akm_suites);
	}
...
struct sprd_chip_ops sc2355_chip_ops = {
	...
	.set_reduce_power = sc2355_set_reduce_power,
	.get_reduce_power = sc2355_get_reduce_power,
};
