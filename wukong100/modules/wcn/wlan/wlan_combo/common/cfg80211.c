...
static void cfg80211_do_work(struct work_struct *work)
{
	...
	while (1) {
		...
		case SPRD_WORK_5G_PW_BACKOFF:
			...
			if (g_set_5g_sar_info.band_sar_supported == FALSE)
				   ...
				   const u8 *mac_addr)
{
	...
	netdev_info(vif->ndev, "%s %s key_index %d, cipher 0x%x\n", __func__,
		    pairwise ? "pairwise" : "group", key_index, cipher);
...
#endif
	sprd_fcc_reset_bo(vif->priv, vif);

	sprd_5g_sar_info_reset(vif->priv);

	/* when receive EVT_ACS_LTE_COMLICT，hostapd will not send
	 * close/open, driver needs to send close/open to make the parameters of
	 * cp2 to 0 before performing the acs scan/start_ap.
	 */
	if (vif->reset_ap_flag) {
		wl_info("close/open to cp2 when reset_ap_flag is true\n");
		...
		if (!ret) {
			if (atomic_read(&hif->power_cnt) == 1) {
				ret = sprd_iface_set_power(hif, false);
				if (ret)
					return ret;
			      ...
			      struct station_del_parameters *params)
{
	...
	netdev_info(ndev, "%s %pM reason:%d\n", __func__, params->mac, params->reason_code);
#else
	netdev_info(ndev, "%s (%02x:%02x:%02x:%02x:xx:xx) reason:%d\n",
		    __func__, params->mac[0], params->mac[1],
		    params->mac[2], params->mac[3], params->reason_code);
#endif
			  ...
			  struct cfg80211_connect_params *sme)
{
	...
	if (!sme->ssid) {
		...
		netdev_info(ndev, "%s %s old_state %d connected(%u) "
                    ...
                    __func__, vif->ssid, old_state, 0, //vif->wdev.connected, #ifdef SRPD_FOR_OLD_KERNEL
...
int sprd_init_fw(struct sprd_vif *vif)
{
	...
	if (vif->mode == SPRD_MODE_AP || vif->mode == SPRD_MODE_P2P_GO) {
		if (vif->has_rand_mac) {
			netdev_dbg(vif->ndev, "use random mac addr:%pM\n",
				    vif->random_mac);
			mac = vif->random_mac;
		}
	}
	...
	set_bit(vif->mode, &priv->active_modes_flags);
...
int sprd_uninit_fw(struct sprd_vif *vif)
{
	...
	clear_bit(vif->mode, &priv->active_modes_flags);
...
struct sprd_priv *sprd_core_create(struct sprd_chip_ops *chip_ops)
{
	...
	sprd_parse_wifi_driver_config(priv);
	sprd_cmd_init(priv);
