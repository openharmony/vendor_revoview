...
struct sprd_chip_ops {
	...
	void (*fcc_reset)(struct sprd_priv *priv, struct sprd_vif *vif);
	void (*fcc_init)(void);
	...
	void (*set_5g_sar_info)(struct sprd_priv *priv, struct sprd_vif * vif, u8 *data, u16 len);
	u8 (*pw_backoff_band2value)(u8 channel);
	int (*set_reduce_power)(struct sprd_vif *vif, unsigned char *data, u16 len);
	int (*get_reduce_power)(struct sprd_vif *vif, unsigned char *data, u16 len,
				unsigned char *rbuf, u16 *rlen);
};
...
static inline void sprd_fcc_reset_bo(struct sprd_priv *priv, struct sprd_vif *vif)
{
	if (priv->chip.ops->fcc_reset)
		return priv->chip.ops->fcc_reset(priv, vif);
}
...
static inline void sprd_abort_scan(struct sprd_priv *priv, struct wiphy *wiphy,
				   struct wireless_dev *wdev)
{
	if (priv->chip.ops->abort_scan)
		priv->chip.ops->abort_scan(wiphy, wdev);
}
...
static inline void sprd_5g_sar_info_set(struct sprd_priv *priv,struct sprd_vif *vif, u8 *data,
					u16 len)
{
	if (priv->chip.ops->set_5g_sar_info)
		priv->chip.ops->set_5g_sar_info(priv, vif, data, len);
}
...
static inline int sprd_set_reduce_power(struct sprd_priv *priv, struct sprd_vif *vif, u8 *data,
					u16 len)
{
	if (priv->chip.ops->set_reduce_power)
		return priv->chip.ops->set_reduce_power(vif, data, len);
	return 0;
}

static inline int sprd_get_reduce_power(struct sprd_priv *priv, struct sprd_vif *vif, u8 *data,
					u16 len, u8 *rbuf, u16 *rlen)
{
	if (priv->chip.ops->get_reduce_power)
		return priv->chip.ops->get_reduce_power(vif, data, len, rbuf, rlen);
