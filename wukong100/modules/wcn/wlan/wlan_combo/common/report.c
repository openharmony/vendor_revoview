...
#else
		netdev_info(vif->ndev, "New station (%02x:%02x:%02x:%02x:xx:xx) connected\n",
...
#else
		netdev_info(vif->ndev, "The station (%02x:%02x:%02x:%02x:xx:xx) disconnected\n",
			    addr[0], addr[1], addr[2], addr[3]);
#endif
	}
}
			    ...
			    struct sprd_connect_info *conn_info, u8 status_code)
{
	...
	if (vif->sm_state != SPRD_CONNECTING &&
		...
		netdev_err(vif->ndev, "%s Unexpected event! connected(%u).\n",
		           __func__, 0); // vif->wdev.connected); #ifdef SRPD_FOR_OLD_KERNEL
...
#else
		netdev_info(vif->ndev, "%s update BSS %s(%02x:%02x:%02x:%02x:xx:xx),"
			    "chn %u, band %d, freq %u, ie_chn(%d).\n", __func__, vif->ssid,
			    conn_info->bssid[0], conn_info->bssid[1], conn_info->bssid[2],
			    conn_info->bssid[3], conn_info->chan, band, freq, ie_channel_number);
#endif
	...
	*/
	if (bss) {
		while (1) {
			...
			if (other_bss && other_bss != bss) {
				wl_debug("unlink bss(%pM-%s) %u that only channel different,"
	...
	netdev_info(vif->ndev, "%s %s to %s (%pM)\n", __func__,
		    ...
		    vif->ssid, vif->bssid);
#else
	netdev_info(vif->ndev, "%s %s to %s (%02x:%02x:%02x:%02x:xx:xx)\n", __func__,
		    status_code == SPRD_CONNECT_SUCCESS ? "connect" : "roam",
		    vif->ssid, vif->bssid[0], vif->bssid[1], vif->bssid[2], vif->bssid[3]);
#endif
...
#else
	netdev_info(vif->ndev,
		    "%s is_mcast:0x%x key_id: 0x%x bssid: %02x:%02x:%02x:%02x:xx:xx\n",
		    __func__, is_mcast, key_id, vif->bssid[0], vif->bssid[1],
		    vif->bssid[2], vif->bssid[3]);
#endif
