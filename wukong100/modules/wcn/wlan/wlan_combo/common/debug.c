...
unsigned int delaycnt1 = 10;
unsigned int delaycnt2 = 4;

void adjust_delaycnt1(char *buf, unsigned char offset)
{
	unsigned int value = 0;
	unsigned int i = 0;
	unsigned int len = strlen(buf) - strlen("delaycnt1=");
	...
	delaycnt1 = value;
	wl_info("%s, change 2G to %d\n", __func__, delaycnt1);
}

void adjust_delaycnt2(char *buf, unsigned char offset)
{
	unsigned int value = 0;
	unsigned int i = 0;
	unsigned int len = strlen(buf) - strlen("delaycnt2=");
...
static struct debug_info_s dbg_info[] = {
	...
	{adjust_delaycnt1, "delaycnt1="},
	{adjust_delaycnt2, "delaycnt2="},
};
			 ...
			 size_t count, loff_t *ppos)
{
	...
	len += scnprintf(buf + len, buflen, "delaycnt1: %d.\n", delaycnt1);
	if (len > buflen)
		len = buflen;

	len += scnprintf(buf + len, buflen, "delaycnt2: %d.\n", delaycnt2);
	if (len > buflen)
		len = buflen;

	len += scnprintf(buf + len, buflen, "tx buff max num: %d.\n", SIPC_TXRX_TX_BUF_MAX_NUM);
	if (len > buflen)
		len = buflen;
