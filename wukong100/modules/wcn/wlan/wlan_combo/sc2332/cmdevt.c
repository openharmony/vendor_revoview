...
static struct sprd_vif *sc2332_find_p2p_vif(struct sprd_priv *priv)
{
	struct sprd_vif *vif, *found = NULL;

	spin_lock_bh(&priv->list_lock);
	list_for_each_entry(vif, &priv->vif_list, vif_node) {
		if (vif->mode == SPRD_MODE_P2P_CLIENT || vif->mode == SPRD_MODE_P2P_GO) {
			vif->ref++;
			found = vif;
			break;
		}
...
int sc2332_set_miracast(struct net_device *ndev, void __user *data)
{
#define MIN_LEN 8
	...
	if (!(priv->extend_feature & SPRD_EXTEND_CMD_MIRACAST)) {
		wl_info("%s, fw not support CMD_MIRACAST", __func__);
		return 0;
	}
	...
	if (subtype == 5) {
		 ...
		 */
		vif = sc2332_find_p2p_vif(priv);
		ret = sc2332_enable_miracast(priv, vif, value);
		sprd_put_vif(vif);
	}
out:
	kfree(command);
...
int sc2332_xmit_data2cmd(struct sk_buff *skb, struct net_device *ndev)
{
	...
	if (vif->mode == SPRD_MODE_P2P_GO || vif->mode == SPRD_MODE_AP) {
		...
		memcpy(hdr->addr3, ehdr.h_dest, ETH_ALEN);
		/* data type:to ds */
		hdr->frame_control = 0x0108;
	}
