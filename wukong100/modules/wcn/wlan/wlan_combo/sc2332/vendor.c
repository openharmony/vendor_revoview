...
static int
vendor_set_sar_limits(struct wiphy *wiphy, struct wireless_dev *wdev,
		      const void *data, int len)
{
/* set sar param according to different scence */
	...
	wl_debug("set_sar sar_scence %u.\n", sar_scence);
	psar_param = sprd_vendor_read_sar_param(sar_scence);
	if (psar_param) {
		netdev_info(vif->ndev, "%s: set sar, scence: %u, value : %d\n",
					__func__, psar_param->sar_scence,
					psar_param->power_value);
		ret = sc2332_set_sar(priv, vif, psar_param->sar_type,
			psar_param->power_value);
		if (ret)
			wl_err("set sar value failed, result: %d", ret);
	} else {
		wl_err("invalid sar scence: %u\n", sar_scence);
	}
