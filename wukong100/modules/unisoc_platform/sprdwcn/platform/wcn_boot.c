...
#endif
static char BTWF_FIRMWARE_PATH[255] = "/vendor/firmware/wcnmodem.bin";
...
static int vddwcn_parent_enable(bool enable)
{
	int ret = -1;

	if (marlin_dev->vddwcn_parent) {
		usleep_range_state(4000, 5000, TASK_UNINTERRUPTIBLE);
		if (enable) {
			ret = regulator_enable(marlin_dev->vddwcn_parent);
			if (ret)
				pr_err("fail to enable vddwcn_parent\n");
			pr_info("vddwcn_parent unbound enable = %d, status = %d\n",
				enable, regulator_is_enabled(marlin_dev->vddwcn_parent));
		} else {
			ret = regulator_disable(marlin_dev->vddwcn_parent);
			if (ret)
				pr_err("fail to disable vddwcn_parent\n");
			pr_info("vddwcn_parent unbound enable = %d, status = %d\n",
				enable, regulator_is_enabled(marlin_dev->vddwcn_parent));
...
{
	if (vddwcn_parent_enable(!enable)) {
		if (gen1_bound_value) {
...
static int wcn_config_wcn_refout_power(struct platform_device *pdev)
{
	marlin_dev->refout_wcn = devm_regulator_get_optional(&pdev->dev, "refout-wcn");
	if (IS_ERR(marlin_dev->refout_wcn)) {
		pr_err("Get refout-wcn err = %ld\n", PTR_ERR(marlin_dev->refout_wcn));
		marlin_dev->refout_wcn = NULL;
	}

	marlin_dev->refout_wcn_xtl2 = devm_regulator_get_optional(&pdev->dev, "refout-wcn-xtl2");
	if (IS_ERR(marlin_dev->refout_wcn_xtl2)) {
		pr_err("Get refout-wcn-xtl2 err = %ld\n", PTR_ERR(marlin_dev->refout_wcn_xtl2));
		marlin_dev->refout_wcn_xtl2 = NULL;
	}

	marlin_dev->dcxo18 = devm_regulator_get_optional(&pdev->dev, "dcxo18");
	if (IS_ERR(marlin_dev->dcxo18)) {
		pr_err("Get dcxo18 err = %ld\n", PTR_ERR(marlin_dev->dcxo18));
		marlin_dev->dcxo18 = NULL;
	}

	marlin_dev->dcxo18_xtl2 = devm_regulator_get_optional(&pdev->dev, "dcxo18-xtl2");
	if (IS_ERR(marlin_dev->dcxo18_xtl2)) {
		pr_err("Get dcxo18-xtl2 err = %ld\n", PTR_ERR(marlin_dev->dcxo18_xtl2));
		marlin_dev->dcxo18_xtl2 = NULL;
	}

	return 0;
}

static int wcn_config_rf_power_clk(struct platform_device *pdev)
{
	marlin_dev->vddrf0v9 = devm_regulator_get_optional(&pdev->dev, "vddrf0v9");
	if (IS_ERR(marlin_dev->vddrf0v9)) {
		pr_err("Get vddrf0v9 err = %ld\n", PTR_ERR(marlin_dev->vddrf0v9));
		marlin_dev->vddrf0v9 = NULL;
	}

	marlin_dev->vddrf1v8 = devm_regulator_get_optional(&pdev->dev, "vddrf1v8");
	if (IS_ERR(marlin_dev->vddrf1v8)) {
		pr_err("Get vddrf1v8 err = %ld\n", PTR_ERR(marlin_dev->vddrf1v8));
		marlin_dev->vddrf1v8 = NULL;
	}

	marlin_dev->vddrf1v1 = devm_regulator_get_optional(&pdev->dev, "vddrf1v1");
	if (IS_ERR(marlin_dev->vddrf1v1)) {
		pr_err("Get vddrf1v1 err = %ld\n", PTR_ERR(marlin_dev->vddrf1v1));
		marlin_dev->vddrf1v1 = NULL;
	}

	marlin_dev->vddldo0 = devm_regulator_get_optional(&pdev->dev, "vddldo0");
	if (IS_ERR(marlin_dev->vddldo0)) {
		pr_err("Get vddldo0 err = %ld\n", PTR_ERR(marlin_dev->vddldo0));
		marlin_dev->vddldo0 = NULL;
	}

	marlin_dev->vddldo4 = devm_regulator_get_optional(&pdev->dev, "vddldo4");
	if (IS_ERR(marlin_dev->vddldo4)) {
		pr_err("Get vddldo4 err = %ld\n", PTR_ERR(marlin_dev->vddldo4));
		marlin_dev->vddldo4 = NULL;
	}

	marlin_dev->refout_rf = devm_regulator_get_optional(&pdev->dev, "refout-rf");
	if (IS_ERR(marlin_dev->refout_rf)) {
		pr_err("Get refout-rf err = %ld\n", PTR_ERR(marlin_dev->refout_rf));
		marlin_dev->refout_rf = NULL;
	}

	return 0;
}

static int wcn_config_wcn_parent(struct platform_device *pdev)
{
	marlin_dev->vddwcn_parent = devm_regulator_get_optional(&pdev->dev, "vddwcn-parent");
	if (IS_ERR(marlin_dev->vddwcn_parent)) {
		pr_err("Get vddwcn-parent err = %ld\n", PTR_ERR(marlin_dev->vddwcn_parent));
		marlin_dev->vddwcn_parent = NULL;
	}
	return 0;
}
...
static int marlin_parse_dt(struct platform_device *pdev)
{
	...
	wcn_config_wcn_refout_power(pdev);
	wcn_config_rf_power_clk(pdev);
	wcn_config_wcn_parent(pdev);
	...
	if (!ret) {
		...
		marlin_dev->crystal_type = clk->type;
...
/*solve bug2487687, wcn impact on rf*/
int marlin_refout_wcn_xtl2_enable(bool enable)
{
	int ret = 0;

	if ((!marlin_dev->refout_wcn_xtl2) ||
		(marlin_dev->crystal_type != WCN_CLOCK_TYPE_TSX))
		return 0;

	if (enable) {
		if (regulator_is_enabled(marlin_dev->refout_wcn_xtl2))
			pr_info("refout_wcn_xtl2 have enable\n");

		ret = regulator_enable(marlin_dev->refout_wcn_xtl2);
		pr_info("refout_wcn_xtl2 enable\n");
		if (ret)
			pr_err("fail to enable refout_wcn_xtl2\n");
	} else {
		if (regulator_is_enabled(marlin_dev->refout_wcn_xtl2))
			pr_info("refout_wcn_xtl2 disable\n");
		ret = regulator_disable(marlin_dev->refout_wcn_xtl2);
		if (ret)
			pr_err("fail to disable refout_wcn_xtl2\n");
	}

	return ret;
}

int marlin_dcxo18_xtl2_enable(bool enable)
{
	int ret = 0;

	if ((!marlin_dev->dcxo18_xtl2) ||
		(marlin_dev->crystal_type != WCN_CLOCK_TYPE_TSX))
		return 0;

	if (enable) {
		if (regulator_is_enabled(marlin_dev->dcxo18_xtl2))
			pr_info("dcxo18_xtl2 have enable\n");

		ret = regulator_enable(marlin_dev->dcxo18_xtl2);
		pr_info("dcxo18_xtl2 enable\n");
		if (ret)
			pr_err("fail to enable dcxo18_xtl2\n");
	} else {
		if (regulator_is_enabled(marlin_dev->dcxo18_xtl2))
			pr_info("dcxo18_xtl2 disable\n");
		ret = regulator_disable(marlin_dev->dcxo18_xtl2);
		if (ret)
			pr_err("fail to disable dcxo18_xtl2\n");
	}

	return ret;
}

int marlin_do_pmic_enable(struct regulator *power, bool enable, char *reg_name)
{
	int ret = 0;
	...
	if (enable) {
		if (regulator_is_enabled(power))
			pr_info("[%s] have enable\n", reg_name);

		ret = regulator_enable(power);
		pr_info("[%s] enable\n", reg_name);
		if (ret)
			pr_err("fail to enable [%s]\n", reg_name);
	} else {
		if (regulator_is_enabled(power))
			pr_info("[%s] disable\n", reg_name);
		ret = regulator_disable(power);
		if (ret)
			pr_err("fail to disable [%s]\n", reg_name);
	}

	return ret;
}

/*solve bug2487687, rf impact on gnss*/
static void wcn_do_rf_power(enum wcn_sub_sys subsys, bool enable)
{
	if ((subsys == MARLIN_GNSS) && (marlin_dev->crystal_type == WCN_CLOCK_TYPE_TSX)) {
		pr_info("WCN_CLOCK_TYPE is TSX, wcn will config rf power and refout1\n");
		marlin_do_pmic_enable(marlin_dev->vddrf0v9, enable, "vddrf0v9");
		marlin_do_pmic_enable(marlin_dev->vddrf1v8, enable, "vddrf1v8");
		marlin_do_pmic_enable(marlin_dev->vddrf1v1, enable, "vddrf1v1");
		marlin_do_pmic_enable(marlin_dev->vddldo0, enable, "vddldo0");
		marlin_do_pmic_enable(marlin_dev->vddldo4, enable, "vddldo4");
		marlin_do_pmic_enable(marlin_dev->refout_rf, enable, "refout_rf");
	}
}
...
static int chip_power_on(enum wcn_sub_sys subsys)
{
	...
	wifipa_enable(1);
	wcn_wifipa_bound_xtl(true);
	marlin_dcxo18_xtl2_enable(true);
	marlin_refout_wcn_xtl2_enable(true);
	if (bus_scan_card() < 0)
		goto wcn_power_off;
	loopcheck_ready_set();
	if (g_match_config && !g_match_config->unisoc_wcn_pcie) {
...
wcn_power_off:
	...
	marlin_refout_wcn_xtl2_enable(false);
	marlin_dcxo18_xtl2_enable(false);
