...
int wcn_power_enable_merlion_domain(bool enable)
{
	...
	WCN_INFO("[%s]enable = %d, btwf_open=%d, gnss_open=%d\n", __func__, 
...
/*
 * gnss_clear_outbox_fifo was used by GNSS
 */
void gnss_clear_outbox_fifo(void)
{
	u32 rd , wt , trigger_data;
	int gnss_outbox_sts;

	trigger_data = 0x1;
	if (wcn_platform_chip_type() == WCN_PLATFORM_TYPE_QOGIRL6) {
		wcn_read_data_from_phy_addr(INTEG_GNSS_OUTBOX_BASE_ADDR,
			&gnss_outbox_sts, sizeof(gnss_outbox_sts));
		rd = ((gnss_outbox_sts >> 24) & 0xFF);
		wt = ((gnss_outbox_sts >> 16) & 0xFF);

		WCN_INFO("[+]%s rd %d, wt %d, sts 0x%x", __func__, rd, wt, gnss_outbox_sts);
trigger_again:
		if (wt != rd)
			wcn_write_data_to_phy_addr(INTEG_GNSS_OUTBOX_TRI_ADDR,
...
EXPORT_SYMBOL_GPL(gnss_clear_outbox_fifo);
/*
...
int wcn_power_enable_dcxo1v8(bool enable)
{
	...
	if (s_wcn_device.dcxo1v8_en_count > 2 ||
		...
		WCN_ERR("[%s]vddwcn_en_count=%d", __func__, s_wcn_device.dcxo1v8_en_count);
...
static void wcn_soft_reset_rfi_master(void)
{
	struct regmap *regmap;
	u32 value;
	...
	if (platform_type == WCN_PLATFORM_TYPE_QOGIRL6) {
		regmap = wcn_get_btwf_regmap(REGMAP_WCN_AON_APB);

		/* soft reset rfi master */
		wcn_regmap_read(regmap, 0x0, &value);
		/* BIT 26: soft reset rfi master */
		value |= 1 << 26;
		wcn_regmap_raw_write_bit(regmap, 0x0, value);

		usleep_range(100, 120);

		/* release soft reset rfi master */
		wcn_regmap_read(regmap, 0x0, &value);
		/* BIT 26: soft reset rfi master */
		value &= ~(1 << 26);
		wcn_regmap_raw_write_bit(regmap, 0x0, value);

		WCN_INFO("%s RFI_MASTER soft reset\r\n", __func__);
	}
}
...
u32 mdbg_check_clean_cache_done(u32 value)
{
	phys_addr_t init_addr;
	u32 val = 0;
