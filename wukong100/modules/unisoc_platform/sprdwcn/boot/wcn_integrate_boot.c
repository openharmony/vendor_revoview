...
void integ_wcn_chip_power_off(void)
{
	if (wcn_platform_chip_type() == WCN_PLATFORM_TYPE_QOGIRL6) {
		...
		atomic_set(&s_wcn_device.btwf_device->reset_open_state,
...
{
	phys_addr_t phy_addr;
	u32 magic_value = 0, temp_magic_value = 0;
	u32 wait_count;
	...
	for (wait_count = 0; wait_count < GNSS_WAIT_CP_INIT_COUNT;
		...
		if (magic_value == GNSS_TEST_MAGIC)
			break;
		msleep(MARLIN_SHORT_WAIT_CP_INIT_POLL_TIME);
		if (magic_value != temp_magic_value) {
			temp_magic_value = magic_value;
...
{
	s_wcn_device.gnss_device->host_direct_access_fw_en = en;
}
EXPORT_SYMBOL_GPL(wcn_set_host_direct_gnss);

bool wcn_get_host_direct_gnss(void)
{
	return s_wcn_device.gnss_device->host_direct_access_fw_en;
}
...
boot_timeout:
	if (unlikely(wcn_dev->boot_cp_status != WCN_BOOT_CP2_OK))
		complete(&wcn_dev->download_done);
}
...
int start_integrate_wcn_truely(u32 subsys)
{
	...
	if (subsys == WCN_MARLIN_MDBG)
		atomic_set(&wcn_dev->reset_open_state, 0);

	if (is_marlin && atomic_read(&wcn_dev->reset_open_state)) {
		WCN_ERR("Reset not end, waiting!");
		mutex_unlock(&wcn_dev->power_lock);
		mutex_unlock(&marlin_lock);
		while (ret < BTWF_SYS_POWERON_WAKEUP_POLLING_COUNT) {
			msleep(5);
			if (!atomic_read(&wcn_dev->reset_open_state))
				break;
			ret++;
		};
		mutex_lock(&marlin_lock);
	...
	if (ret_wait_completion <= 0) {
		...
		if (wcn_dev->boot_cp_status == WCN_BOOT_CP2_ERR_BOOT)
			goto err_boot_marlin;
	}
	wcn_dev->wcn_open_status |= subsys_bit;
...
{
	static u32 first_time, retry_time = 0;
	u32 btwf_subsys;
	u32 ret;
	...
	if (ret == -ETIMEDOUT) {
		if (subsys == WCN_GNSS || subsys == WCN_GNSS_BD || subsys == WCN_GNSS_GAL) {
			if (!retry_time) {
				...
				wcn_assert_interface(WCN_SOURCE_GNSS, "GNSS sys boot cp timeout");
		} else
			wcn_assert_interface(WCN_SOURCE_BTWF, "BTWF sys boot cp timeout");
	}
	return ret;
}
...
int stop_integrate_wcn_module(u32 subsys)
{
	...
	if (unlikely(!(subsys_bit & wcn_dev->wcn_open_status))) {
		/* It wants to stop not opened device */
		if (subsys_bit & atomic_read(&wcn_dev->reset_open_state)) {
			WCN_INFO("After reset close, status %d\n", subsys_bit
				& atomic_read(&wcn_dev->reset_open_state));
			ret = atomic_read(&wcn_dev->reset_open_state) & ~subsys_bit;
			atomic_set(&wcn_dev->reset_open_state, ret);
		}
		WCN_ERR("%s not opend, err: subsys = %d\n",
			wcn_dev->name, subsys);
